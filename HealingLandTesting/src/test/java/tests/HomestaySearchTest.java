...
public class HomestaySearchTest {
    ...
    @Test
    public void testCheckInDateCannotBeInPast() {
        // Get today's date in the format of the input field
        String today = LocalDate.now().format(DateTimeFormatter.ISO_LOCAL_DATE);
        String pastDate = LocalDate.now().minusDays(1).format(DateTimeFormatter.ISO_LOCAL_DATE);

        // Find the check-in date input field and set a past date
        WebElement checkInInput = driver.findElement(By.id("checkIn"));
        checkInInput.sendKeys(pastDate);

        // Check if the value of check-in date input is today's date or a future date
    ...
    @Test
    public void testCheckOutDateCannotBeEarlierThanCheckInDate() {
        // Get today's date and a future date in the format of the input field
        String futureDate = LocalDate.now().plusDays(5).format(DateTimeFormatter.ISO_LOCAL_DATE);
        String earlierDate = LocalDate.now().plusDays(3).format(DateTimeFormatter.ISO_LOCAL_DATE);
    ...
    @Test
    public void testCheckInDateCannotBeLaterThanCheckOutDate() {
        // Get today's date and a future date in the format of the input field
        String validCheckOutDate = LocalDate.now().plusDays(5).format(DateTimeFormatter.ISO_LOCAL_DATE);
        String invalidCheckInDate = LocalDate.now().plusDays(6).format(DateTimeFormatter.ISO_LOCAL_DATE);
